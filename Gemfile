source "https://rubygems.org"

gem 'susy'
gem 'compass'
gem 'rake'
gem 'modular-scale'

group :test do 
  
end

group :development do 
  gem 'guard'
  gem 'guard-livereload'
  gem 'guard-compass'
  gem 'pry'
  gem 'rb-fsevent' #faster compiler without polling  
  gem 'yajl-ruby' #faster communication between livereload and guard
end

# you also might need to ... gem install animation --pre

# gem "compass_twitter_bootstrap", "2.2.1" # does not exist yet at this version but you can build it yourself from commit 36624333ed on compass_twitter_bootstrap project on github.
# https://github.com/vwall/compass-twitter-bootstrap